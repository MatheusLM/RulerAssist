...
ipcRenderer.on(
  "sync",
  (event, newTheme, newData, newControls, newRulerData, newGridData) => {
    theme = newTheme;
    data = newData;
    ...
    grid.style.visibility =
      gridData.show && !controls.help ? "visible" : "hidden";
...
ipcRenderer.on("syncRuler", (event, newRulerData) => {
  rulerData = newRulerData;
  rulerEquivalent.value = rulerData.equivalent;
  markers.style.backgroundColor = rulerData.equivalentRuler
    ? "rgba(160,255,160,0.65)"
    : "transparent";
});
...
ipcRenderer.on("sendControls", (event, newControls, newData) => {
  ...
  // MARKERS
  stepSize = rulerData.equivalentRuler
    ? rulerData.equivalent / controls.markers
    : data.width / controls.markers;
  ...
  if (controls.symmetrical) {
    ...
    stepSize = rulerData.equivalentRuler
      ? rulerData.equivalent / 2 / markersCount
      : data.width / 2 / markersCount;
  ...
  for (let i = 0; i < markersNumber.length; i++) {
    ...
    let spanText = controls.showSizes
      ? step + ":" + Math.floor(step * stepSize)
      : step;
...
function updateGridLines() {
  ...
  let stepHeight = rulerData.equivalentRuler
    ? gridData.heightEquivalent / gridData.rows
    : gridData.height / gridData.rows;
  let stepWidth = rulerData.equivalentRuler
    ? gridData.widthEquivalent / gridData.columns
    : gridData.width / gridData.columns;

  for (let step = 0; step <= gridData.rows; step++) {
    let div = document.createElement("div");
    let text = document.createElement("span");
    let heightText = controls.showSizes
      ? step + ":" + Math.floor(step * stepHeight)
      : step;
  ...
  for (let step = 0; step <= gridData.columns; step++) {
    let div = document.createElement("div");
    let text = document.createElement("span");
    let widthText = controls.showSizes
      ? step + ":" + Math.floor(step * stepWidth)
      : step;
