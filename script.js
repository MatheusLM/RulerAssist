...
let horizontalMarkers = horizontal.getElementsByClassName('horizontal-line');
...
ipcRenderer.on('sendTheme', (event, newData) => {
    ...
    // Rotates
    ruler.style.transform = theme.rotated ? 'rotateZ(-180deg)' : '';
    gridContainer.style.transform = theme.rotated ? 'rotateZ(-180deg)' : '';
    controlsRuler.style.transform = theme.rotated ? 'rotateZ(-180deg)' : '';

    horizontalMarkers = horizontal.getElementsByClassName('horizontal-line');
    for (let i = 0; i < horizontalMarkers.length; i++) {
        horizontalMarkers[i].style.transform = theme.rotated ? 'rotateZ(-180deg)' : '';
        horizontalMarkers[i].children[0].style.transform = theme.rotated ? 'translateX(-95%) translateY(-50%)' : '';
        horizontalMarkers[i].style.justifyContent = theme.rotated ? 'flex-start' : 'flex-end';
    }
});
